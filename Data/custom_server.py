...
class SlashesMiddleware:
	...
	async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
		if scope['type'] == 'http' and scope['path'].startswith("/mcp") and not scope['path'].endswith("/"):
	...
	async def handle_streamable_http(self, scope: Scope, receive: Receive, send: Send) -> None:
		...
		if not auth_header or not auth_header.startswith('Bearer '):
			...
			response = Response(
				...
				headers={
					"WWW-Authenticate": f'Bearer realm="MCP API", resource_metadata="{base_url}/.well-known/oauth-protected-resource"',
		...
		if token not in self.oauth.access_tokens:
		...
		token_data = self.oauth.access_tokens[token]
		import time
