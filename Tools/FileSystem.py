...
class FileSystemTools:
    ...
    def _validate_path(self, path: str) -> str:
        ...
        if any(char in normalized for char in forbidden_chars):
            raise ValueError(
                f"Путь содержит запрещенные символы: {forbidden_chars}")
        ...
        try:
            ...
            if not real_target_path.startswith(real_working_dir):
                raise ValueError(
                    "Попытка доступа за пределы рабочей директории")
        except OSError:
    ...
    def _register_tools(self, mcp: MCP_Tools):
        ...
        def savefile(
            ...
            try:
                ...
                self.logger.info(
                    f"File saved: {path} ({self._format_size(size)})")
                return [
                    types.TextContent(type="text",
                                      text=f"Файл '{path}' успешно сохранен")
                ]
            except ValueError as e:
                error_msg = f"Ошибка валидации пути: {str(e)}"
                self.logger.warning(
                    f"Path validation failed for '{path}': {str(e)}")
                return [types.TextContent(type="text", text=error_msg)]
            except Exception as e:
        ...
        def readfile(
            ...
            try:
                ...
                self.logger.info(
                    f"File read: {path} ({self._format_size(len(content.encode(encoding)))})"
                )
        ...
        def deletefile(
            ...
            try:
                ...
                if not os.path.exists(safe_path):
                    error_msg = f"Файл '{path}' не найден"
                    self.logger.warning(
                        f"Attempted to delete non-existent file: {path}")
                    return [types.TextContent(type="text", text=error_msg)]

                if os.path.isdir(safe_path):
                    error_msg = f"'{path}' является директорией. Используйте deletedir для удаления директорий"
                    self.logger.warning(
                        f"Attempted to delete directory as file: {path}")
                    return [types.TextContent(type="text", text=error_msg)]
                ...
                return [
                    types.TextContent(type="text",
                                      text=f"Файл '{path}' успешно удален")
                ]
            except ValueError as e:
                error_msg = f"Ошибка валидации пути: {str(e)}"
                self.logger.warning(
        ...
        def createdir(
            ...
            try:
                ...
                if os.path.exists(safe_path):
                    ...
                    self.logger.warning(
                        f"Attempted to create existing directory: {path}")
                    return [types.TextContent(type="text", text=error_msg)]
                ...
                return [
                    types.TextContent(
                        type="text",
                        text=f"Директория '{path}' успешно создана")
                ]
            except ValueError as e:
                error_msg = f"Ошибка валидации пути: {str(e)}"
                self.logger.warning(
        ...
        def deletedir(
            path: Annotated[str, "Путь к директории"],
            recursive: Annotated[
                bool, "Рекурсивное удаление (удалить все содержимое)"] = False
        ) -> list[types.TextContent | types.ImageContent
                  | types.EmbeddedResource]:
            try:
                safe_path = self._get_safe_path(path)
                ...
                if not os.path.exists(safe_path):
                    ...
                    self.logger.warning(
                        f"Attempted to delete non-existent directory: {path}")
                    return [types.TextContent(type="text", text=error_msg)]

                if not os.path.isdir(safe_path):
                    error_msg = f"'{path}' не является директорией"
                    self.logger.warning(
                        f"Attempted to delete non-directory as directory: {path}"
                    )
                    return [types.TextContent(type="text", text=error_msg)]
                ...
                if recursive:
                    ...
                    return [
                        types.TextContent(
                            type="text",
                            text=
                            f"Директория '{path}' и все её содержимое успешно удалены"
                        )
                    ]
                else:
                    os.rmdir(safe_path)
                    ...
                    return [
                        types.TextContent(
                            type="text",
                            text=f"Пустая директория '{path}' успешно удалена")
                    ]
            except OSError as e:
                ...
                self.logger.error(
                    f"Failed to delete directory '{path}': {str(e)}")
                return [types.TextContent(type="text", text=error_msg)]
            except ValueError as e:
                error_msg = f"Ошибка валидации пути: {str(e)}"
                self.logger.warning(
        ...
        def fileinfo(
            ...
            try:
                ...
                if not os.path.exists(safe_path):
                    ...
                    self.logger.warning(
                        f"Attempted to get info for non-existent path: {path}")
                    return [types.TextContent(type="text", text=error_msg)]
                ...
                mtime = datetime.datetime.fromtimestamp(
                    stat.st_mtime, self.tz_plus3)
                ctime = datetime.datetime.fromtimestamp(
                    stat.st_ctime, self.tz_plus3)

                info = {
                    "Путь": path,
                    "Тип": "Директория" if is_dir else "Файл",
                    "Размер":
                    self._format_size(stat.st_size) if not is_dir else "-",
                    "Последнее изменение":
        ...
        @mcp.register_tool(
            name="getlogs",
            description="Получить логи операций файловой системы")
        def getlogs() -> list[types.TextContent | types.ImageContent
                              | types.EmbeddedResource]:
            try:
                ...
                if not logs.strip():
                    return [
                        types.TextContent(type="text", text="Лог-файл пуст")
                    ]
